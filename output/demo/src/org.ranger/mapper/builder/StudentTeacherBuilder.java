...
/**
 * tb_student_teacher表Provider
 * @author ranger
 *
 */
public class StudentTeacherProvider {
	...
	public String batchInsert(List<Map<String,Object>> entitis){
		...
		String sql="INSERT INTO tb_student_teacher(tea_id)  VALUES";
		StringBuilder sb=new StringBuilder(sql);
		for(int i=0;i<entitis.size();i++){
			sb.append("(#{list["+i+"].tea_id})");
	...
	public String deleteByIds(Map<String, Object[]> ids) {
		...
		String sql = "DELETE FROM tb_student_teacher WHERE stu_id IN ";
		StringBuilder sb = new StringBuilder(sql);
	 ...
	 * @Description 批量更新记录
	 * @param entitis
	 *            -->tea_id= #{list["+i+"].tea_id}
	 * @return
	 */
	public String batchUpdate(List<Map<String,Object>> entitis){
		StringBuilder sql = new StringBuilder();
		for(int i=0;i<entitis.size();i++){
			sql.append("UPDATE tb_student_teacher SET tea_id= #{list["+i+"].tea_id}");
			if(i<(entitis.get("list").size()-1)){
				sql.append(",");
			}
	...
	public String findByIds(Map<String,Object[] > ids){
		...
		String sql = "SELECT * FROM tb_student_teacher WHERE MODULAR_ID IN ";
		StringBuilder sb = new StringBuilder(sql);
		sb.append(Arrays.toString(ids.get("array")));
	...
	public String findByEntity(Map<String, Object> entity){
		String base = "SElECT * FROM tb_student_teacher WHERE 1=1 ";
	...
	public String count(Map<String, Object> entity){
		String base = "SELECT count(*) as total FROM tb_student_teacher a WHERE 1=1";
		StringBuilder sql = new StringBuilder(base);
		if(entity != null){
			  if(entity.get("stu_id")!=null){
			 sql.append("stu_id=#{stu_id}");
